<template>  
    ...
    <ProTable
        ...
        <el-button
            ...
            link
            :icon="Switch"
            v-hasPermi="['sys:lead:toCustomer']"
            @click="toCustomer(scope.row.id)"
            v-if="scope.row.status === LeadStatus.NOT_CONVERTED"
            >
            转为客户
        </el-button>
...
//获取 ProTable 元素调用其获取刷新数据方法（还能获取到当前查询参数，方便导出携带参数）
const proTable = ref()
...
const openFollowDrawer=(title:string,row:Partial<any> ={})=>{
    let params={
        ...
        getTableList:proTable.value.getTableList,maxHeight:'450px'
    }
